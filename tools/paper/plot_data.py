...
def calculateShearThinningJointExp(t, xs, P, guess):
    ...
    def eq(t, *params):
        ...
        A, T, K12, K3, n = params
        xsEst = A * (1 - np.exp(T * (t - minTime)))
        vsEst = -A * T * np.exp(T * (t - minTime))
        PEst = (K12 * (vsEst)) ** 1.2
        PEst -= PEst[0]
        return ((xsEst - xs) / xsRange)**2 + ((PEst - P) / PRange)**2
    params = opt.curve_fit(eq, t, np.zeros(t.shape), guess)
    return params
...
if __name__ == "__main__":
    ...
    for key, metadata in metadataDict.items():
        ...
        A, T, K12, K3, n = calculateShearThinningJointExp(*interp(xsTail.data, pressureTail.data), (200, -1e-1, 1, 1, 1.2))[0]
        print(A, T, K12, K3, n)
        xsFit = A * (1 - np.exp(T * (xsTail.data[:, 0] - min(xsTail.data[:, 0]))))
        vsFit = -A * T * np.exp(T * (xsTail.data[:, 0] - min(xsTail.data[:, 0])))
        ...
        # n[key] = pressureParams[2] / xsParams[2]
